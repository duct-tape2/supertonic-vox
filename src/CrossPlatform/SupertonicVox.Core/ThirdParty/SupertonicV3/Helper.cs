// Vendored from supertone-inc/supertonic v3.0.0 (commit 5379cc4e4297cec249a8a71283fc44d55ec327f1).\n// Upstream source is MIT licensed. See licenses/Supertonic-Python_MIT.txt and THIRD_PARTY_NOTICES.md.\nusing System;
...
namespace Supertonic
{
    ...
    public class TextToSpeech : IDisposable
    {
            ...
            CancellationToken cancellationToken = default)
        {
            ...
            using var dpRunOptions = new RunOptions();
            using var dpCancellation = cancellationToken.Register(() => dpRunOptions.Terminate = true);
            using var dpOutputs = _dpOrt.Run(dpInputs, _dpOrt.OutputMetadata.Keys.ToArray(), dpRunOptions);
            ...
            if (durOnnx.Any(duration => !float.IsFinite(duration) || duration <= 0 || duration > maxDurationSeconds))
            {
                throw new InvalidDataException("Predicted audio duration is outside the allowed range.");
            }
            ...
            using var textEncoderRunOptions = new RunOptions();
            using var textEncoderCancellation = cancellationToken.Register(() => textEncoderRunOptions.Terminate = true);
            using var textEncOutputs = _textEncOrt.Run(
                textEncInputs,
                _textEncOrt.OutputMetadata.Keys.ToArray(),
                textEncoderRunOptions);
            ...
            for (int step = 0; step < totalStep; step++)
            {
                ...
                using var vectorRunOptions = new RunOptions();
                using var vectorCancellation = cancellationToken.Register(() => vectorRunOptions.Terminate = true);
                using var vectorEstOutputs = _vectorEstOrt.Run(
                    vectorEstInputs,
                    _vectorEstOrt.OutputMetadata.Keys.ToArray(),
                    vectorRunOptions);
            ...
            using var vocoderRunOptions = new RunOptions();
            using var vocoderCancellation = cancellationToken.Register(() => vocoderRunOptions.Terminate = true);
            using var vocoderOutputs = _vocoderOrt.Run(
                vocoderInputs,
                _vocoderOrt.OutputMetadata.Keys.ToArray(),
                vocoderRunOptions);
            ...
            CancellationToken cancellationToken = default)
        {
            ...
            {
                cancellationToken.ThrowIfCancellationRequested();
                var remainingDuration = maxDurationSeconds - durCat;
                if (remainingDuration <= 0) throw new InvalidDataException("Maximum audio duration was exceeded.");
                var (wav, duration) = _Infer(
        ...
        public (float[] wav, float[] duration) Batch(
            ...
            CancellationToken cancellationToken = default)
        {
            return _Infer(textList, langList, style, totalStep, speed, maxDurationSeconds, cancellationToken);
        }
